...
Page({
  ...
  httpLaunch: function() {
    ...
    if (newdata.username == "" && newdata.password.length == "") {
      wx.showToast({
        title: '学号密码为空',
        image: '/images/x.png'
      })
    } else if (newdata.password.length == "" && newdata.username.length != "") {
      wx.showToast({
        title: '密码不能为空',
        image: '/images/x.png'
      })
      ...
      wx.request({
        url: 'http://jwnew.cqust.edu.cn/eams/login.action',
        data: newdata,
        header: {
        ...
        success: res => {
          ...
          if (res.data.indexOf('火狐浏览器') == -1) {
            wx.showModal({
              title: '提示',
              content: '学生认证需要自拍照一张，确认是否继续',
              success: function(x) {
                if (x.confirm) {
                  console.log('用户点击确定');
                  ...
                  //console.log('用户点击确定');
                  first = res.data.indexOf('<a href=\"/eams/security/my.action\" target=\"_blank\" title=\"查看登录记录\" style=\"color:#ffffff\">');
                  end = res.data.indexOf('</a>', first);
                  var pages = getCurrentPages(); //从页面路由栈中直接获取和操作目标Page对象
                  var name = res.data.substring(first + 88, end);
                  that.setData({
                    nameXY: name,
                  })
                  ...
                  wx.navigateTo({
                    url: '../logon_user_picture/logon_user_picture' //跳转到人脸图片上传界面
                  });
  ...
  onShow: function() {
    let that = this;
    if (that.data.tempImagePictureFileID != '') {
      wx.showLoading({
        title: '认证中',
      });
